...
import {
  ...
  ExternalLink, Zap, Shield, Tag, Loader2, X
} from 'lucide-react'
    ...
    <div className="p-6 h-full flex">
      ...
      <div className="w-96 flex-shrink-0 flex flex-col gap-4 overflow-auto pr-4 border-r border-white/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {stats && <span className="text-xs text-muted-foreground">({stats.total_skills})</span>}
          </div>
          <button
        ...
        <div className="flex flex-col gap-2">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 rounded-lg pl-8 pr-3 py-1.5 text-sm focus:outline-none focus:border-purple-500/50"
            />
          </div>
          <div className="flex gap-2">
            ...
            <select
              ...
              className="bg-white/5 border border-white/10 rounded-lg px-2 py-1 text-xs focus:outline-none flex-1"
            >
        ...
        {loading && skills.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {filtered.map(skill => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${TYPE_COLORS[skill.skill_type] || 'bg-gray-500/20 text-gray-300'}`}>
                    {skill.skill_type}
                  </span>
                </div>
            ...
            {filtered.length === 0 && (
              <div className="text-center py-8 text-muted-foreground text-sm">No skills found</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 pl-6 overflow-auto">
        {selectedSkill ? (
          <SkillDetail
            skill={selectedSkill}
            onBind={() => setShowBind(true)}
            onUnbind={(agentId, skillId) => unbindSkill(agentId, skillId)}
            onDelete={() => deleteSkill(selectedSkill.id)}
          />
        ) : (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-16 h-16 rounded-2xl bg-purple-500/10 flex items-center justify-center mb-4">
              <Puzzle size={32} className="text-purple-400/50" />
            </div>
      ...
      {showBind && selectedSkill && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowBind(false)}>
          <div className="bg-[#1a1b1e] rounded-2xl p-6 w-[400px] border border-white/10" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              <input value={bindForm.agent_id} onChange={e => setBindForm(f => ({ ...f, agent_id: e.target.value }))} placeholder="Agent ID" className="bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-purple-500/50" />
              <div className="flex items-center gap-2">
                <label className="text-sm text-muted-foreground">Confidence:</label>
                <input type="range" min="0" max="1" step="0.1" value={bindForm.confidence} onChange={e => setBindForm(f => ({ ...f, confidence: parseFloat(e.target.value) }))} className="flex-1" />
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full ${TYPE_COLORS[skill.skill_type]}`}>
              {skill.skill_type}
            </span>
      ...
      {skill.tags && skill.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {skill.tags.map(tag => (
            <span key={tag} className="flex items-center gap-1 px-2 py-0.5 bg-white/5 rounded-full text-xs text-muted-foreground">
              <Tag size={10} /> {tag}
            </span>
          ))}
        </div>
      ...
      {skill.prompt_template && (
        <div>
          ...
          <pre className="p-3 rounded-xl bg-black/30 border border-white/5 text-xs font-mono whitespace-pre-wrap text-white/70 max-h-48 overflow-auto">
            {skill.prompt_template}
          </pre>
        </div>
      )}
          ...
          <div className="flex flex-col gap-1.5">
            {skill.bindings.map(binding => (
              <div key={binding.id} className="flex items-center justify-between p-2.5 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <button
                  onClick={() => onUnbind(binding.agent_id, skill.id)}
                  className="flex items-center gap-1 text-xs text-red-400/60 hover:text-red-400 transition"
                >
